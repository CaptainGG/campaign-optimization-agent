<script lang="ts">
  ...
  $: activeRecs = recommendations.filter(r => r.action !== 'NoAction');
...
<div class="panel">
  ...
  {:else}
    ...
    <div class="rec-list">
      {#each sorted as rec}
        <div class="rec-item" class:no-action={rec.action === 'NoAction'}>
          <div class="rec-top">
...
<style>
  ...
  .empty-icon { font-size: 28px; color: #2a3040; }

  .empty p { font-size: 13px; color: #4a5260; line-height: 1.5; }
  ...
  .empty button:disabled { opacity: 0.5; cursor: not-allowed; }
  ...
  .summary-count { font-size: 22px; font-weight: 700; color: #ff7043; }
  .summary-label { font-size: 12px; color: #6a7280; }
  ...
  .all-clear-icon { color: #4fffb0; font-size: 14px; }
  ...
  .rec-item:hover { background: #141720; }
  .rec-item.no-action { opacity: 0.45; }
  ...
  .rec-campaign { font-size: 12px; font-weight: 600; color: #9ca3b0; font-family: monospace; flex: 1; }
  .rec-urgency { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; }

  .rec-reason { font-size: 12px; color: #5a6270; line-height: 1.5; }
  ...
  .change-field { color: #6a7280; }
  .change-arrow { color: #3a4050; }
  .change-to { color: #4fffb0; font-weight: 600; }
</style>
